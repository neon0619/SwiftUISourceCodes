...
//
//  Created by Christopher Castillo on 9/15/22.
//
...
class CreateViewModelFailureTests: XCTestCase {
    ...
    func test_with_unsuccessful_response_submission_state_is_unsuccessful() async throws {
        ...
        XCTAssertEqual(vm.error, .networking(error: NetworkingManager.NetworkingError.invalidURL), "The view model error should be a networking error with invalidUrl")
    }
